            ...
            <div>
              <Reveal>
                <p className="max-w-3xl text-lg leading-relaxed text-navy-700">
                  Working across multiple jurisdictions for thirty years has
                  made us genuinely versatile. We&apos;ve encountered
                  challenges that most firms simply haven&apos;t — and that
                  experience means we spot things early, ask the right
                  questions, and bring solutions that actually hold up.
                </p>
              </Reveal>
              <Reveal delay={0.08}>
                <dl className="mt-10 flex flex-wrap gap-x-12 gap-y-6 border-t border-navy-100 pt-8">
                  {[
                    [`${new Date().getFullYear() - 1996}`, "Years in practice"],
                    ["7,000+", "Projects delivered"],
                    ["3", "Offices"],
                  ].map(([value, label]) => (
                    <div key={label}>
                      <dd className="font-heading text-4xl font-bold text-navy-900">
                        {value}
                      </dd>
                      <dt className="mt-1 text-xs font-semibold uppercase tracking-wider text-navy-500">
                        {label}
                      </dt>
                    </div>
                  ))}
